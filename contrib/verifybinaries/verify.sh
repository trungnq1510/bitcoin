...
###   bitnamicoincore.org and bitnamicoin.org and compares them.
...
WORKINGDIR="/tmp/bitnamicoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitnamicoincore.org"
HOST2="https://bitnamicoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitnamicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitnamicoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "bitnamicoin.org failed to provide signature file, but bitnamicoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitnamicoin.org and bitnamicoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitnamicoin Core binary release signing key installed?"
   fi
