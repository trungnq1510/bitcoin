Bitnamicoin Core version 0.11.0 is now available from:

  <https://bitnamicoin.org/bin/bitnamicoin-core-0.11.0/>
...
For example, add the following to `bitnamicoin.conf`:
...
related to the blockchain in the bitnamicoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Bitnamicoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Bitnamicoin Core will 
...
For an example script see [bitnamicoin-submittx](https://github.com/laanwj/bitnamicoin-submittx).
...
- #5334 `cf87536` libbitnamicoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bitnamicoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BitnamicoinAmountField
- #5753 `0f44672` Add bitnamicoin logo to about screen
...
- #5809 `46bfbe7` Add bitnamicoin-cli man page
...
- #5849 `41259ca` contrib/init/bitnamicoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Bitnamicoin-Qt.app to Bitnamicoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitnamicoin-tx)
