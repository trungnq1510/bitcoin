...
The Bitnamicoin Core daemon can be configured to act as a trusted "border
router", implementing the bitnamicoin wire protocol and relay, making
...
The ZeroMQ feature in Bitnamicoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitnamicoind:
...
For instance:
    ...
    $ bitnamicoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/bitnamicoind.tx.raw \
...
These options can also be provided in bitnamicoin.conf.
...
From the perspective of bitnamicoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitnamicoind directly. Furthermore, no information is
...
using. Bitnamicoind appends an up-counting sequence number to each
