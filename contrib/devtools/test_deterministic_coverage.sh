...
TEST_BITNAMICOIN_BINARY="src/test/test_bitnamicoin"
...
if [[ ! -e ${TEST_BITNAMICOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_BITNAMICOIN_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_BITNAMICOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
