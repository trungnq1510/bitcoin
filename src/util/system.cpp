...
const char * const BITNAMICOIN_CONF_FILENAME = "bitnamicoin.conf";
const char * const BITNAMICOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bitnamicoin/bitnamicoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitnamicoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BITNAMICOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "bitnamicoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitnamicoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitnamicoin
    // Mac: ~/Library/Application Support/Bitnamicoin
    // Unix: ~/.bitnamicoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitnamicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitnamicoin";
#else
    // Unix
    return pathRet / ".bitnamicoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITNAMICOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Bitnamicoin Core copyright is not removed by accident
    if (copyright_devs.find("Bitnamicoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bitnamicoin Core developers";
    }
